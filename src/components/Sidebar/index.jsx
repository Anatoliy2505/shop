...
const Sidebar = ({ catalog }) => {
	...
	const createTree = (catalog, itemId, cb) => {
				...
				<li key={item.id} className={'sidebar-catalog__parent ' + (itemId === item.id ? 'active' : '')}>
					<div className={'sidebar-catalog__parent-wrap'}>
						<a className={'sidebar-catalog__item'} href={'/' + item.name}>
							<span className={'sidebar-catalog__parent-img'} />
							{item.title}
						</a>
	...
	return (
		<aside className={'sidebar'}>
			<nav className={'sidebar-catalog'}>
				...
				<ul className={'sidebar-catalog__list'}>{createTree(catalog, activeItem, onSelectItem)}</ul>
			</nav>
		</aside>
	);
};
